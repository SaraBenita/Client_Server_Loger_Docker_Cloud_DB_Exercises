...
public class ServletsController {
    ...
    public ResponseEntity<ServerResponse> updateBooksPrice(@RequestParam Integer id, @RequestParam Integer price) {
        ...
        if (optionalBook == null) {
            ...
            booksLogger.debug("Book [{}] price change: {} --> {}", optionalBook.getTitle(), optionalBook.getPrice(), price);
    ...
    public ResponseEntity<ServerResponse> deleteBook(@RequestParam Integer id) {
        ...
        if (optionalBook == null) {
            ...
            booksLogger.debug("After removing book [{}] id: [{}] there are {} books in the system", optionalBook.getTitle(), optionalBook.getId(), serverUtils.getSetBooks().size()-1);
    ...
    public ResponseEntity<String> getLoggerLevel(@RequestParam(name = "logger-name") String loggerName) {
        try {
            int requestNumber = incrementRequestCounter();
            long startTime = System.currentTimeMillis();
            requestLogger.info("Incoming request | #{} | resource: /logs/level | HTTP Verb GET", requestNumber);
    ...
    public ResponseEntity<String> setLoggerLevel(@RequestParam(name = "logger-name") String loggerName,
        ...
        try {
            int requestNumber = incrementRequestCounter();
            long startTime = System.currentTimeMillis();
            requestLogger.info("Incoming request | #{} | resource: /logs/level | HTTP Verb PUT", requestNumber);
